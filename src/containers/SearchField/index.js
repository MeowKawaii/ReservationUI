...
class SearchField extends Component {
  ...
  searchBooking() {
    if (this.state.text == "") {
      ...
      fetch(
        `http://localhost:8080/bookings/searching?type=${
          this.state.title
      ...
      <Fragment>
        <Grid container direction="coloum" justify="center" alignItems="center">
          <div className="search">
            ...
            <Button
              handle={this.searchBooking}
              text={"Search"}
              variant={"outlined"}
            />
            <Button
              handle={this.handleOpenModel}
              text={"Booking"}
              variant={"contained"}
            />
          </div>
        ...
        <MyFormDialog
          ...
          handleCloseModal={this.handleOpenModel}
          handleRight={this.bookingRoom}
        />
      </Fragment>
    );
  }
